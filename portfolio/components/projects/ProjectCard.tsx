    ...
    >
      ...
      <div className="p-6 h-full flex flex-col justify-between border border-transparent group-hover:border-primary transition-all duration-300">
        ...
        <div>
          ...
          {/* Short Description */}
          <p className="text-text mb-4">{project.shortDescription}</p>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {project.technologies.slice(0, 3).map((tech, index) => (
              <span
                key={index}
                className="px-2 py-1 text-xs font-medium bg-secondary/20 text-text rounded-md"
              >
                {tech}
              </span>
            ))}
        ...
        <div className="flex justify-between items-center">
          {/* Timeframe */}
          <span className="text-sm text-accent">{project.timeframe}</span>
