...
const ExperienceCard: React.FC<ExperienceCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute left-0 md:left-1/2 transform md:translate-x-[-50%] top-6 md:top-8 z-10"
      ...
      >
        <motion.div
          className={`ml-8 md:ml-0 p-6 rounded-lg bg-accent/10 border border-accent/30 transition-all duration-300 hover:border-primary relative text-left ${
            isEven ? "md:mr-6" : "md:ml-6"
          }`}
          initial={{
            opacity: 0,
            x: isEven ? -100 : 100, // Different entry direction based on position
          }}
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center mb-4 gap-3">
            <motion.div
              className="flex-shrink-0 w-12 h-12 bg-accent/20 rounded-full overflow-hidden relative flex items-center justify-center border-accent border-1"
            ...
            >
              ...
              <div className="text-sm text-text">{experience.location}</div>
            </motion.div>
          </div>
          ...
          >
            <div className="flex flex-wrap gap-2 justify-start">
              {experience.skills.map((skill, index) => (
                <motion.span
                  key={index}
                  className="px-2 py-1 text-xs bg-secondary/20 text-text rounded-md"
